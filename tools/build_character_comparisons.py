...
for gender in ('male','female'):
    for view in ('front','side'):
        ...
        reference=Image.open(OUT/f'validated_reference_{gender}_{view}.png').convert('RGB')
        result_mask=mask(result,115); ref_mask=mask(reference,100)
        ...
        ref_mask=mask(reference,100); fb=ref_mask.getbbox()
        ...
        aligned.paste(reference,(dx,dy),ref_mask); alpha.paste(ref_mask,(dx,dy))
        aligned.save(OUT/f'clean_reference_{gender}_{view}.png')
        overlay=Image.new('RGB',result.size,(58,58,58)); overlay.paste((64,190,255),(0,0,result.width,result.height),result_mask); overlay.paste((255,85,85),(0,0,result.width,result.height),alpha)
