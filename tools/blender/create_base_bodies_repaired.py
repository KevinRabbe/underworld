...
def build_male_topology():
    ...
    for s in (-1,1):
        ...
        _segment_surface(verts,faces,[(s*.30,.08,1.50),(s*.47,.08,1.39),(s*.66,.08,1.18),(s*.75,.06,1.10),(s*.82,.045,1.065),(s*.87,.035,1.045),(s*.92,.030,1.040)],[.095,.068,.050,.034,.038,.022,.017],12,True,True)
        _segment_surface(verts,faces,[(s*.16,.08,.62),(s*.18,.08,.56),(s*.195,.08,.47),(s*.19,.08,.38),(s*.19,.08,.12)],[.160,.145,.100,.085,.050],12,True,True)
    ...
    for vertex in obj.data.vertices:
        ...
        if y < 0.02 and 1.68 <= z <= 1.90:
            ...
            vertex.co.y -= 0.068 * nose + 0.026 * brow + 0.034 * chin
