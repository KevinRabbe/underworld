...
const GameSaveSnapshotBuilderScript := preload("res://app/game/session/game_save_snapshot_builder.gd")
...
func build_save_request() -> Dictionary:
	return GameSaveSnapshotBuilderScript.capture(
		world_settings,
		_session_world_context,
		_world_session_state,
		world_delta_store,
		survival,
		player,
