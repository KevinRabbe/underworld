...
static func compose_combat(
	...
	var hunting_controller = BoarEncounterControllerScript.new()
	hunting_controller.name = "BoarEncounters"
	root.add_child(hunting_controller)
	hunting_controller.configure(world, player, world_settings, null)
	...
	return {
		...
		"hunting_controller": hunting_controller,
