...
if "const COMPILER_REVISION := 7" not in compiler:
    raise SystemExit("expected compiler revision 7")
compiler = compiler.replace("const COMPILER_REVISION := 7", "const COMPILER_REVISION := 8", 1)

torso = r'''static func _build_torso(builders: Dictionary, profile, outfit: Resource) -> void:
...
compiler = replace_function(compiler, "static func _build_torso", "static func _build_head", torso)

old_shoulder_cap = r'''	# Keep the vest shoulder seam compact so the beige sleeve remains the main
...
'''
if old_shoulder_cap not in compiler:
    raise SystemExit("expected old shoulder cap block")
compiler = compiler.replace(old_shoulder_cap, "", 1)

outfit = r'''static func _build_outfit_details(builders: Dictionary, profile) -> void:
	var landmark: Dictionary = profile.anatomy_landmarks()
	var pelvis_y: float = float(landmark["pelvis_y"])
	var shoulder_y: float = float(landmark["shoulder_y"])
	var chest_y: float = float(landmark["chest_y"])
	...
	_add_y_loft(builders, CANVAS_DARK, [
		_ring_y(Vector3(0, waist_y, 0.205), 0.135, 0.075, _weights(BONE.chest)),
		_ring_y(Vector3(0, lerpf(waist_y, lower_chest_y, 0.55), 0.220), 0.175, 0.100, _weights(BONE.chest)),
		_ring_y(Vector3(0, chest_y - 0.030, 0.225), 0.185, 0.105, _weights(BONE.chest)),
	...
	_add_box(builders, CANVAS_LIGHT, Vector3(-0.155, chest_y - 0.105, 0.306), Vector3(0.155, chest_y + 0.045, 0.326), _weights(BONE.chest), 0.008)
	_add_box(builders, CANVAS_DARK, Vector3(-0.222, waist_y + 0.035, 0.210), Vector3(-0.170, waist_y + 0.155, 0.292), _weights(BONE.chest), 0.012)
	_add_box(builders, CANVAS_DARK, Vector3(0.170, waist_y + 0.035, 0.210), Vector3(0.222, waist_y + 0.155, 0.292), _weights(BONE.chest), 0.012)
	_add_box(builders, CANVAS_LIGHT, Vector3(-0.220, waist_y + 0.140, 0.206), Vector3(-0.172, waist_y + 0.172, 0.296), _weights(BONE.chest), 0.006)
	_add_box(builders, CANVAS_LIGHT, Vector3(0.172, waist_y + 0.140, 0.206), Vector3(0.220, waist_y + 0.172, 0.296), _weights(BONE.chest), 0.006)
	for strap_x in [-0.115, 0.115]:
		_add_ribbon_z(builders, LEATHER, Vector2(strap_x, waist_y + 0.02), Vector2(strap_x, shoulder_y - 0.02), 0.350, 0.018, _weights(BONE.chest), _weights(BONE.chest), 1.0)
		_add_box(builders, METAL,
			Vector3(strap_x - 0.020, lower_chest_y - 0.010, 0.349),
			Vector3(strap_x + 0.020, lower_chest_y + 0.026, 0.358),
			_weights(BONE.chest), 0.004)
	_add_ribbon_z(builders, LEATHER_LIGHT, Vector2(-0.14, chest_y - 0.02), Vector2(0.14, chest_y - 0.02), 0.352, 0.012, _weights(BONE.chest), _weights(BONE.chest), 1.0)
...
compiler = replace_function(compiler, "static func _build_outfit_details", "static func _joint_overlap_margins", outfit)

helper_marker = "static func _add_x_loft(builders: Dictionary"
if "static func _add_y_loft_open_front" not in compiler:
    helper = r'''static func _add_y_loft_open_front(builders: Dictionary, palette_index: int, rings: Array[Dictionary], sides: int) -> void:
	if rings.size() < 2:
		return
...
'''
    compiler = compiler.replace(helper_marker, helper + helper_marker, 1)

prism_marker = "static func _add_tri_prism_z(builders: Dictionary"
if "static func _add_quad_prism_z" not in compiler:
    quad_helper = r'''static func _add_quad_prism_z(builders: Dictionary, palette_index: int, a: Vector2, b: Vector2, c: Vector2, d: Vector2, front_z: float, back_z: float, skin_weights: Dictionary) -> void:
...
'''
    compiler = compiler.replace(prism_marker, quad_helper + prism_marker, 1)

palette_replacements = {
    '_entry("skin", Color("9b6749"), 0.80, 0.0)': '_entry("skin", Color("bf8b6a"), 0.80, 0.0)',
    '_entry("cloth", Color("565247"), 0.93, 0.0)': '_entry("cloth", Color("80684d"), 0.93, 0.0)',
    '_entry("leather", Color("3e2a20"), 0.78, 0.0)': '_entry("leather", Color("4b3023"), 0.78, 0.0)',
    '_entry("accent", Color("347a86"), 0.84, 0.0)': '_entry("accent", Color("3f8790"), 0.84, 0.0)',
    '_entry("canvas_light", Color("776b54"), 0.95, 0.0)': '_entry("canvas_light", Color("c4b59a"), 0.95, 0.0)',
    '_entry("trouser", Color("323532"), 0.92, 0.0)': '_entry("trouser", Color("444a49"), 0.92, 0.0)',
    '_entry("canvas_dark", Color("48443a"), 0.96, 0.0)': '_entry("canvas_dark", Color("5f5544"), 0.96, 0.0)',
    '_entry("leather_light", Color("5a3b29"), 0.84, 0.0)': '_entry("leather_light", Color("765037"), 0.84, 0.0)',
}
