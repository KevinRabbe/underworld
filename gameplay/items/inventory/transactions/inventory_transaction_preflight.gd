...
func analyze(plan) -> Dictionary:
	...
	for operation in operations:
		descriptors.append(operation_descriptor(operation))
...
static func operation_descriptor(operation: Dictionary) -> Dictionary:
	var definition = operation.get("definition", null)
...
static func _operation_label(operation: Dictionary) -> String:
	var descriptor: Dictionary = operation_descriptor(operation)
	return "%s[%s:%s]" % [
		descriptor.get("kind", ""),
