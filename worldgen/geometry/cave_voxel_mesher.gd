...
static func build(request) -> StageResult:
	...
	var metrics := {"fragment_count": fragments.size(), "source_descriptor_count": _unique_count(descriptor_ids), "vertex_count": vertices.size(), "index_count": indices.size(), "triangle_count": indices.size() / 3, "sample_count": samples, "cube_count": cubes, "sample_pitch": pitch, "sample_halo": request.partition_configuration.sample_halo, "marching_cubes_table_revision": MARCHING_CUBES_TABLE_REVISION, "extraction_mode": "global_signed_distance_marching_cubes", "memory_bytes": memory_bytes, "extraction_ms": float(Time.get_ticks_usec() - started) / 1000.0, "preparation_ms": float(Time.get_ticks_usec() - started) / 1000.0}
