...
const PointerModeTests := preload("res://tests/presentation/test_pointer_mode_controller_contract.gd")
...
func _run() -> void:
	...
	failures.append_array(PointerModeTests.run())
	...
	if failures.is_empty():
		...
		print("  routing / off-tree NEW+CONTINUE / pause-resume / Save & Quit / teardown / theme boundary / pointer arbitration passed")
		quit(0)
		return
