...
static func run_runtime(tree: SceneTree) -> Array[String]:
	...
	if tree_body != null:
		...
		if player.global_position.distance_to(tree_target_position) > 1.0:
			player.global_position = tree_target_position - target_direction.normalized() * 0.5
	...
	_expect(failures, "three real tree clicks produce canonical wood", not tree_object_id.is_empty() and inventory.quantity_of("item.resource.wood") >= wood_before_harvest + 4)
	...
	if world != null and world.has_method("find_nearest_pickup_position"):
		var fiber_position: Vector3 = world.call("find_nearest_pickup_position", player.global_position, "plant_fiber")
		if fiber_position != Vector3.INF:
			player.global_position = fiber_position
