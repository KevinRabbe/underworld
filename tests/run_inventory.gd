...
const InventoryTransactionPreflightTests := preload("res://tests/inventory/test_inventory_transaction_preflight.gd")
...
func _init() -> void:
	...
	failures.append_array(InventoryTransactionPreflightTests.run())
	...
	failures.append_array(FoodConsumptionTests.run(self))
	if failures.is_empty():
		print("[INVENTORY VALIDATION] PASS")
		print("  container / authored-definition-identity / transaction / transaction-preflight / equipment-hotbar / surface-harvest / loot-collection / skinning / food contracts passed")
		quit(0)
		return
