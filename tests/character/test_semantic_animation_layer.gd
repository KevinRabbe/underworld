...
static func _test_two_sets_and_rigs_share_semantics(failures: Array[String]) -> void:
	...
	var rig_a = _rig_fixture("rig_profile.humanoid.test_a", "root_a", "hand_a")
	var rig_b = _rig_fixture("rig_profile.humanoid.test_b", "root_b", "hand_b")
	var set_a = _animation_set_fixture("animation_set.humanoid.test_a", rig_a.content_id, "pack_a")
	var set_b = _animation_set_fixture("animation_set.humanoid.test_b", rig_b.content_id, "pack_b")
	var fixture: Dictionary = _validated_content([set_b, rig_a, set_a, rig_b])
	...
	var diagnostics_a: Array[String] = controller_a.configure(
		fixture["registry"], fixture["validation"], role_registry, set_a.content_id, adapter_a
	)
	var diagnostics_b: Array[String] = controller_b.configure(
		fixture["registry"], fixture["validation"], role_registry, set_b.content_id, adapter_b
	)
	...
	_expect_equal(failures, "gameplay attack semantic role", controller_a.semantic_role_for_action(&"attack"), "animation_role.action.attack.light_01")
	_expect_equal(failures, "left dodge semantic role", controller_a.semantic_role_for_action(&"dodge", Vector2(-1.0, 0.0)), "animation_role.action.dodge.left")

	var gameplay = PlayerActionController.new(TestStamina.new())
	_expect_true(failures, "gameplay attack starts independently of animation set", gameplay.try_start_attack(0.12, 0.08, 0.30))
	...
	_expect_equal(failures, "set A concrete attack binding", adapter_a.last_played, "pack_a.attack")
	_expect_equal(failures, "set B concrete attack binding", adapter_b.last_played, "pack_b.attack")
	_expect_equal(failures, "set A receives gameplay duration", float(adapter_a.last_parameters.get("duration", -1.0)), authoritative_duration)
	_expect_equal(failures, "set B receives gameplay duration", float(adapter_b.last_parameters.get("duration", -1.0)), authoritative_duration)
	_expect_equal(failures, "presentation does not change gameplay timing", gameplay.get_attack_total_duration(), authoritative_duration)
...
static func _test_missing_role_diagnostics(failures: Array[String]) -> void:
	var role_registry = CharacterSemanticSchemaCatalog.build_registry()
	var rig = _rig_fixture("rig_profile.humanoid.bad_role", "root_bad", "hand_bad")
	...
	_expect_true(failures, "unknown semantic role fails clearly", _has_fragment(diagnostics, "unknown animation role schema id"))
	_expect_true(failures, "unknown role leaves controller unconfigured", not controller.is_ready())
	adapter.dispose()
...
static func _test_authored_prototype_runtime(failures: Array[String]) -> void:
	...
	_expect_true(failures, "prototype semantic controller ready", controller != null and controller.is_ready())
	if controller == null or not controller.is_ready():
		mannequin.free()
		return

	_expect_equal(failures, "prototype animation set id", controller.animation_set_id(), "animation_set.humanoid.prototype")
	_expect_equal(failures, "prototype rig profile id", controller.rig_profile_id(), "rig_profile.humanoid.prototype")
	_expect_equal(failures, "semantic parry concrete binding", controller.animation_binding_for_role("animation_role.action.parry"), "prototype.parry")
	_expect_equal(failures, "explicit death fallback", controller.animation_binding_for_role("animation_role.reaction.death"), "prototype.hit.front")
	_expect_true(failures, "semantic right-hand socket resolves", controller.resolve_rig_node("rig_role.socket.hand.right") == mannequin.get_socket(&"hand_r"))
	_expect_true(failures, "semantic attachment root resolves", controller.attachment_root("rig_role.socket.hand.right") == mannequin.get_tool_visual_root())

	controller.present_attack(0.70)
	_expect_equal(failures, "semantic attack reaches mannequin", mannequin.current_action, mannequin.ACTION_ATTACK)
	controller.update_locomotion(0.45, Vector3.ZERO, 0.0, true, false)
	_expect_equal(failures, "gameplay attack duration still active", mannequin.current_action, mannequin.ACTION_ATTACK)
	controller.update_locomotion(0.30, Vector3.ZERO, 0.0, true, false)
	_expect_equal(failures, "gameplay attack duration ends", mannequin.current_action, mannequin.ACTION_NONE)
	controller.present_death()
	_expect_equal(failures, "death fallback reaches hit presentation", mannequin.current_action, mannequin.ACTION_HIT)
	mannequin.free()
...
static func _validated_content(definitions: Array) -> Dictionary:
	...
	var validation: Dictionary = ContentValidationPipeline.new().validate_all(definitions, categories, capabilities)
...
static func _rig_fixture(content_id: String, root_target: String, hand_target: String):
...
static func _animation_set_fixture(content_id: String, rig_profile_id: String, prefix: String):
	var animation_set = AnimationSetDefinition.new()
	animation_set.configure_animation_set(content_id, rig_profile_id)
	animation_set.role_bindings = {
...
static func _adapter_for_set(animation_set, root_target: String, hand_target: String):
	var adapter = RecordingAdapter.new()
