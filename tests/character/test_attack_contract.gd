...
static func _test_attack_catalog(failures: Array[String]) -> void:
	...
	_expect_true(failures, "prototype attack profiles can vary timing", not is_equal_approx(float(hands.call("total_duration")), float(pickaxe.call("total_duration"))))

	var execution: Dictionary = axe.call("make_execution", Vector3(3.0, 4.0, 5.0), Vector3(0.0, 8.0, -4.0))
	...
	_expect_vector_close(failures, "execution stores normalized horizontal direction", execution.get("direction", Vector3.ZERO), Vector3(0.0, 0.0, -1.0))
...
static func _test_attack_phase_controller(failures: Array[String]) -> void:
	...
	_expect_true(failures, "attack commitment rejects dodge", not actions.try_start_dodge(Vector3.RIGHT))
...
static func _test_attack_phase_edge_cases(failures: Array[String]) -> void:
	...
	_expect_true(failures, "large-frame crossing preserves one activation", actions.consume_attack_activation())
	_expect_true(failures, "large-frame activation remains one-shot", not actions.consume_attack_activation())
	...
	_expect_true(failures, "reset discards pending attack activation", not actions.consume_attack_activation())
...
static func _test_live_player_activation(tree: SceneTree, failures: Array[String]) -> void:
	...
	_expect_equal(failures, "live RMB enters attack startup", String(player.call("get_action_state_name")), "ATTACKING/STARTUP")
	...
	if captured.size() == 1:
		_expect_equal(failures, "committed attack keeps original axe id", captured[0].get("attack_id"), &"stone_axe_light")
		_expect_equal(failures, "committed attack keeps original axe damage", int(captured[0].get("damage", 0)), 16)
		...
		_expect_equal(failures, "CombatManager consumes supplied execution", String(combat.call("get_last_combat_message")), "Attack missed")
