...
static func run(tree: SceneTree) -> Array[String]:
	...
	_expect_true(failures, "player parry action starts", bool(actions.call("try_start_parry")))
	actions.call("tick", 0.07)
	var health_before: int = int(player.call("get_health"))
	var parry_result: StringName = player.call("receive_melee_attack", 10, front_source, true)
	_expect_equal(failures, "frontal active parry resolves as parried", parry_result, &"parried")
	_expect_equal(failures, "parry prevents health loss", int(player.call("get_health")), health_before)
	...
	var rear_parry_result: StringName = player.call("receive_melee_attack", 10, rear_source, true)
	_expect_equal(failures, "rear melee bypasses active parry", rear_parry_result, &"hit")
	_expect_equal(failures, "rear melee damages parrying player", int(player.call("get_health")), rear_parry_health - 10)

	actions.call("reset")
	stamina.call("reset")
	player.set("damage_invulnerability_timer", 0.0)
	_expect_true(failures, "player dodge action starts", bool(actions.call("try_start_dodge", Vector3(1.0, 0.0, 0.0))))
	actions.call("tick", 0.10)
	var dodge_health_before: int = int(player.call("get_health"))
	var dodge_result: StringName = player.call("receive_melee_attack", 10, rear_source, true)
	_expect_equal(failures, "active dodge iframe resolves as dodged", dodge_result, &"dodged")
	_expect_equal(failures, "dodge iframe prevents health loss", int(player.call("get_health")), dodge_health_before)
	...
	_expect_true(failures, "player block state reaches mannequin guard pose", bool(mannequin.call("is_block_pose_active")))
	var guard_yaw: float = visual_root.rotation.y
	player.set("velocity", Vector3(4.0, 0.0, 0.0))
	player.call("_update_visual_facing", 1.0)
	_expect_close(failures, "held guard preserves facing while strafing", visual_root.rotation.y, guard_yaw)
	player.set("velocity", Vector3.ZERO)

	var block_health_before: int = int(player.call("get_health"))
	var block_result: StringName = player.call("receive_melee_attack", 10, front_source, true)
	_expect_equal(failures, "front melee resolves as blocked", block_result, &"blocked")
	_expect_equal(failures, "successful block prevents health loss", int(player.call("get_health")), block_health_before)
	...
	var rear_result: StringName = player.call("receive_melee_attack", 10, rear_source, true)
	_expect_equal(failures, "rear melee bypasses block", rear_result, &"hit")
	_expect_equal(failures, "rear melee damages blocking player", int(player.call("get_health")), rear_health_before - 10)
	...
	var break_result: StringName = player.call("receive_melee_attack", 10, front_source, true)
	_expect_equal(failures, "unaffordable block resolves attack as hit", break_result, &"hit")
	_expect_equal(failures, "guard-break hit reduces health", int(player.call("get_health")), break_health_before - 10)
	...
	player.call("_update_mannequin", 1.0 / 60.0)
	_expect_true(failures, "guard break clears mannequin guard pose", not bool(mannequin.call("is_block_pose_active")))

	actions.call("reset")
	stamina.call("reset")
	player.set("damage_invulnerability_timer", 0.0)
	var normal_health_before: int = int(player.call("get_health"))
	var hit_result: StringName = player.call("receive_melee_attack", 10, front_source, true)
	_expect_equal(failures, "undefended melee resolves as hit", hit_result, &"hit")
	_expect_equal(failures, "undefended melee reduces health", int(player.call("get_health")), normal_health_before - 10)
	...
	_expect_true(failures, "tool action establishes camera-facing combat direction", attack_forward.dot(camera_forward) > 0.999)
	_expect_equal(failures, "live player exposes USING_TOOL state", String(player.call("get_action_state_name")), "USING_TOOL")
	_expect_true(failures, "live tool commitment rejects dodge", not bool(actions.call("try_start_dodge", Vector3.RIGHT)))
	...
	_expect_close(failures, "tool cooldown and commitment finish together", float(player.get("tool_use_cooldown_timer")), 0.0)
