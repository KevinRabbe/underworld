...
static func run() -> Array[String]:
	...
	var before_empty_demand: int = incarnations.records.size()
	var empty_result = incarnations.set_demand(incarnation_address, "walker", [])
	_expect(failures, "empty desired demand cannot recreate evicted record", empty_result == null and incarnations.records.size() == before_empty_demand)
