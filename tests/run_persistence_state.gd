...
const PendingLootRestoreAllocatorTests := preload("res://tests/persistence/test_pending_loot_restore_allocator.gd")
...
func _run() -> void:
	...
	failures.append_array(PendingLootRestoreAllocatorTests.run())
	failures.append_array(IntegratedGameRuntimeLifecycleTests.run_runtime(self))
	if failures.is_empty():
		print("[PERSISTENCE STATE VALIDATION] PASS")
		print("  gameplay codecs / integrated detached save schema / typed wire / atomic slot lifecycle / source-level legacy retirement / restored loot allocator / real Game Continue activation passed")
		quit(0)
		return
