...
func _init() -> void:
	...
	if failures.is_empty():
		...
		print("  semantic content ids / deterministic registry / category-capability-role schemas / headless validation + snapshot evidence / archetype realization / item / resource / creature / weapon / production weapon runtime + failure atomicity / reserved-site assignment / underground placement / surface-harvest / loot / crafting rulebook contracts passed")
		quit(0)
		return
